...
CREATE OR REPLACE PROCEDURE add_film(title VARCHAR(255), description VARCHAR, release_year INTEGER, language_id INTEGER, rental_duration INTEGER, rental_rate NUMERIC(4,2), length INTEGER, replacement_cost NUMERIC(5,2))
LANGUAGE plpgsql
AS $$
BEGIN 
...
CALL add_film('Adaptation', 'A Movie About Orchids and Writing That Will Probably Make You Feel Weird', 2000, 1, 5, 4.99, 100, 19.99);

CALL add_film('Zoo Animals', 'A Movie About Zoo Animals', 1995, 1, 4, 5.99, 130, 14.99);
